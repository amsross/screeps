...
module.exports = function() {
  ...
  if ((Game.spawns.Spawn1 || {}).energy >= 300) {
    ...
    if ( false && Game.spawns.Spawn1.room.find(FIND_HOSTILE_CREEPS, {
      ...
      return Game.spawns.Spawn1.createCreep( [TOUGH, TOUGH, TOUGH, TOUGH, MOVE, ATTACK, MOVE, ATTACK], 'Guard' + Date.now(), {role: 'guard'} );
    }

    if ( !Object.keys( Game.creeps ).length ) {
      return Game.spawns.Spawn1.createCreep( [WORK, WORK, CARRY, MOVE], 'harvester' + Date.now(), {role: 'harvester'} );
    } else if ( spawnDeciderByRole( "miner" ) ) {
      return Game.spawns.Spawn1.createCreep( [WORK, WORK, MOVE, MOVE], 'miner' + Date.now(), {role: 'miner'} );
    } else if ( spawnDeciderByRole( "scavenger" ) ) {
      return Game.spawns.Spawn1.createCreep( [CARRY, CARRY, CARRY, MOVE, MOVE, MOVE], 'scavenger' + Date.now(), {role: 'scavenger'} );
    } else if ( spawnDeciderByRole( "guard" ) ) {
      return Game.spawns.Spawn1.createCreep( [TOUGH, TOUGH, TOUGH, TOUGH, MOVE, ATTACK, MOVE, ATTACK], 'guard' + Date.now(), {role: 'guard'} );
    } else if ( spawnDeciderByRole( "healer" ) ) {
      return Game.spawns.Spawn1.createCreep( [MOVE, HEAL], 'healer' + Date.now(), {role: 'healer'} );
    } else if ( spawnDeciderByRole( "archer" ) ) {
      return Game.spawns.Spawn1.createCreep( [TOUGH, TOUGH, TOUGH, TOUGH, TOUGH, MOVE, MOVE, RANGED_ATTACK], 'archer' + Date.now(), {role: 'archer'} );
    } else if ( spawnDeciderByRole( "conquistador" ) ) {
      return Game.spawns.Spawn1.createCreep( [MOVE, WORK, WORK, CARRY], 'conquistador' + Date.now(), {role: 'conquistador'} );
    } else if ( spawnDeciderByRole( "builder" ) && Game.rooms.sim.find(FIND_CONSTRUCTION_SITES).length ) {
      return Game.spawns.Spawn1.createCreep( [WORK, WORK, CARRY, MOVE], 'builder' + Date.now(), {role: 'builder'} );
    } else {
      return Game.spawns.Spawn1.createCreep( [MOVE, WORK, WORK, CARRY], 'conquistador' + Date.now(), {role: 'conquistador'} );
    }
  }
